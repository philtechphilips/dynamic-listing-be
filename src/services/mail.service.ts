...
function getResendFrom(): string {
    ...
    const name = process.env.RESEND_FROM_NAME || process.env.SMTP_FROM_NAME || "Dynamic Listing";
    return `"${name}" <${email}>`;
}
...
function getFromAddress(): string {
    if (process.env.RESEND_FROM_EMAIL) {
        const name = process.env.SMTP_FROM_NAME || process.env.RESEND_FROM_NAME || "Dynamic Listing";
 ...
 */
export const sendMailInBackground = (to: string, subject: string, html: string, text?: string) => {
